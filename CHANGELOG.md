...
- **测试护栏**：新增 10 个测试套件（原子写、墓碑、防跨站、客户端配置原子写、同步配置、同步服务端存储、HTTP 集成、连通性探测、跨进程写锁、本地自动备份），`npm test` 现跑 12 个套件 / 255 用例。其中的并发用例会拉起 4 个子进程同时对同一集合做读改写，关掉写锁后该用例必定失败，可反向验证锁确实生效。
