...
const CLIENT_DEFS = [
  { type: 'claude', name: 'Claude Desktop',
    ...
    exes: ['{LOCALAPPDATA}/AnthropicClaude/claude.exe', '{LOCALAPPDATA}/AnthropicClaude/claude.cmd'],
  ...
  { type: 'cursor', name: 'Cursor',
    ...
    exes: ['{LOCALAPPDATA}/Programs/cursor/Cursor.exe', '{LOCALAPPDATA}/cursor/Cursor.exe', 'C:/Program Files/Cursor/Cursor.exe'],
  ...
  { type: 'vscode', name: 'VS Code',
    ...
    exes: ['{LOCALAPPDATA}/Programs/Microsoft VS Code/Code.exe', 'C:/Program Files/Microsoft VS Code/Code.exe'],
...
function scanRepoLocal(repo, usePath, resourceType) {
  ...
  if (resourceType === 'skillrepos') {
    ...
    for (const entry of fs.readdirSync(dir)) {
      ...
      const size = fs.statSync(sk).size;
      if (size > MAX_FILE) { warnings.push(`跳过超大文件 ${sk}（${Math.round(size / 1024)}KB）`); continue; }
      const fm = parseSkillFrontMatter(fs.readFileSync(sk, 'utf8'));
      skills.push({ name: fm.name || entry, description: fm.description || '', path: fp, clientType: repo.name, sourcePath: dir, hash: hashOf(fs.readFileSync(sk, 'utf8')) });
    }
