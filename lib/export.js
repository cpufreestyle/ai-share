...
// 原子写客户端配置文件：写同目录临时文件 → fsync → rename（同分区原子）。
// 目标是用户真实配置（claude_desktop_config.json / mcp.json），直接覆盖一旦
// 中途崩溃/磁盘满会截断，导致别的程序读取失败；写前把当前内容留作 .bak 便于回滚。
...
module.exports = { exists, writeJsonAtomic, exportProfile, applyExport, expand, buildMcpServers, detectClients, scanClientMcp, scanMcpConfig, normalizeMcp, scanClientSkills, parseSkillFrontMatter, scanClientPrompts, scanPromptsFromSources, scanRepoLocal, syncRepo, getProxyUrl, gitArgsWithProxy };
