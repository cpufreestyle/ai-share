    ...
    try {
      ...
      if (fs.existsSync(target)) {
        try {
          ...
          // 目标不是合法 JSON（可能含注释/尾逗号的 JSONC，或文件已损坏）：
          // 跳过而非覆盖，避免把用户整份客户端配置清空。
          results.push({ clientId: cfg.clientId, clientName: cfg.clientName, ok: false, error: '目标配置不是合法 JSON，已跳过以避免覆盖：' + target });
          continue;
        }
      }
...
let _ROOTS = null;
// 是否运行在 WSL 内：客户端是 Windows 程序，需要把 {APPDATA}/{USERPROFILE} 映射到
// Windows 用户目录在 WSL 挂载点下的真实路径（/mnt/c/Users/<user>），否则会落到 WSL 根文件系统，
// 导致 aishare_detect_clients 报 WSL 路径、aishare_apply_profile 写入错误位置。
function isWSL() {
...
function getRoots() {
  ...
  let home = process.env.USERPROFILE || process.env.HOME || '';
  let appData = process.env.APPDATA;
  let localAppData = process.env.LOCALAPPDATA;
  if (isWSL()) {
    // WSL 内不会注入 Windows 环境变量，直接用挂载点下的 Windows 用户目录构造
    const uname = (os.userInfo().username || process.env.USER || '').trim();
    const winHome = '/mnt/c/Users/' + uname;
    home = winHome;
...
// WSL 的 /mnt/c 挂载区分大小写，而客户端内置路径模板多为小写（如 Programs/cursor），
// 与 Windows 实际目录名（Programs/Cursor）不一致时会被误判为「不存在」。
// 这里在常规检查失败后，逐级做大小写不敏感回退匹配。
const _ciCache = new Map();
function resolveCaseInsensitive(p) {
  if (_ciCache.has(p)) return _ciCache.get(p);
  let cur = p.startsWith('/') ? '' : '.';
  let ok = true;
  for (const part of p.split('/')) {
    if (!part || part === '.') continue;
    const next = cur + '/' + part;
    if (fs.existsSync(next)) { cur = next; continue; }
    let hit = null;
    try {
      const entries = fs.readdirSync(cur || '/');
      hit = entries.find((e) => e.toLowerCase() === part.toLowerCase()) || null;
    } catch (e) { hit = null; }
    if (!hit) { ok = false; break; }
    cur = cur + '/' + hit;
  }
  const r = ok ? cur : null;
  if (_ciCache.size > 5000) _ciCache.clear();
  _ciCache.set(p, r);
  return r;
}
function exists(p) {
  if (!p) return false;
  try { if (fs.existsSync(p)) return true; } catch (e) { return false; }
  // Windows / macOS 本身大小写不敏感，无需回退；相对路径也不参与（避免误配）
  if (process.platform === 'win32' || process.platform === 'darwin' || p[0] !== '/') return false;
  try { return !!resolveCaseInsensitive(p); } catch (e) { return false; }
}
...
// 参照 terraform plan / chezmoi diff：只计算「会写入什么」，不落盘。
// 返回里所有疑似密钥值都先脱敏再返回（apply 才写真实值），避免明文经 /api 传出。
function planExport(profileId) {
  const data = exportProfile(profileId);
  if (!data) return { ok: false, error: 'profile 不存在' };

  const SECRET_KEY_RE = /(api[-_]?key|secret|token|password|authorization)/i;
  const maskVal = (v) => { const s = String(v); return s.length <= 8 ? '***' : s.slice(0, 4) + '…***' + s.slice(-4); };
  const maskDeep = (o) => {
    if (Array.isArray(o)) return o.map(maskDeep);
    if (o && typeof o === 'object') {
      const out = {};
      for (const k of Object.keys(o)) out[k] = SECRET_KEY_RE.test(k) ? maskVal(o[k]) : maskDeep(o[k]);
      return out;
    }
    return o;
  };
  ...
  const targets = [];
  let added = 0, changed = 0, unchanged = 0, warnings = 0;
  for (const cfg of data.configs) {
    const t = {
      clientId: cfg.clientId, clientName: cfg.clientName, clientType: cfg.clientType,
      path: cfg.configPath, status: 'ok',
      mcpServers: { add: [], change: [], same: [], kept: [] },
      env: { add: [], change: [], same: [] },
      preview: null,
    };
    let existing = null;
    if (!cfg.configPath) t.status = 'no-path';
    else if (!fs.existsSync(cfg.configPath)) t.status = 'new-file';
    else {
      try { existing = JSON.parse(fs.readFileSync(cfg.configPath, 'utf8')); }
      catch (e) { t.status = 'invalid-json'; warnings++; } // apply 会跳过这类文件
    }

    const wantServers = cfg.content.mcpServers || {};
    const wantEnv = cfg.content.env || {};
    if (t.status === 'ok') {
      const curServers = (existing && existing.mcpServers) || {};
      for (const name of Object.keys(wantServers)) {
        if (!(name in curServers)) { t.mcpServers.add.push({ name, after: maskDeep(wantServers[name]) }); added++; }
        else if (same(curServers[name], wantServers[name])) { t.mcpServers.same.push(name); unchanged++; }
        else { t.mcpServers.change.push({ name, before: maskDeep(curServers[name]), after: maskDeep(wantServers[name]) }); changed++; }
      }
      for (const name of Object.keys(curServers)) if (!(name in wantServers)) t.mcpServers.kept.push(name); // apply 是合并，不会动它们
      const curEnv = (existing && existing.env) || {};
      for (const k of Object.keys(wantEnv)) {
        if (!(k in curEnv)) { t.env.add.push({ key: k, after: maskVal(wantEnv[k]) }); added++; }
        else if (String(curEnv[k]) === String(wantEnv[k])) { t.env.same.push(k); unchanged++; }
        else { t.env.change.push({ key: k, before: maskVal(curEnv[k]), after: maskVal(wantEnv[k]) }); changed++; }
      }
      const merged = Object.assign({}, existing, { mcpServers: Object.assign({}, curServers, wantServers) });
      if (Object.keys(wantEnv).length) merged.env = Object.assign({}, curEnv, wantEnv);
      t.preview = maskDeep(merged);
    } else if (t.status === 'new-file') {
      for (const name of Object.keys(wantServers)) { t.mcpServers.add.push({ name, after: maskDeep(wantServers[name]) }); added++; }
      for (const k of Object.keys(wantEnv)) { t.env.add.push({ key: k, after: maskVal(wantEnv[k]) }); added++; }
      t.preview = maskDeep(cfg.content);
    }
    targets.push(t);
  }
  return { ok: true, profileId, profileName: data.profile && data.profile.name, summary: { targets: targets.length, added, changed, unchanged, warnings }, targets };
}
...
module.exports = { exists, writeJsonAtomic, exportProfile, applyExport, planExport, planExport, planExport, planExport, planExport, planExport, planExport, planExport, expand, buildMcpServers, detectClients, scanClientMcp, scanMcpConfig, normalizeMcp, scanClientSkills, parseSkillFrontMatter, scanClientPrompts, scanPromptsFromSources, scanRepoLocal, syncRepo, getProxyUrl, gitArgsWithProxy };
