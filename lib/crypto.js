...
const APP_ROOT = process.pkg
  ...
  : path.join(__dirname, '..', '..');
const KEY_FILE = path.join(APP_ROOT, 'data', '.key');
...
// 判断一个值是否已是密文。写入路径据此避免「把密文再加密一次」：
// store.writeRaw 每次写盘都会 seal 整个集合，而 update/remove/restore 等路径
// 合并的是已加密的原始记录，缺少这个判断就会每保存一次多套一层壳
// （历史 bug：某条 apiKey 被套了 30 多层，从 30 字节膨胀到 3 MB）。
function isSealed(v) { return typeof v === 'string' && v.startsWith('v1:'); }

module.exports = { encrypt, decrypt, isSealed, getKey, setKey, clearKey, hasKey, deriveKey, genSalt };
