...
const os = require('os');
const { encrypt, decrypt, isSealed } = require('./crypto');
...
// 种子数据中的示例根目录：Windows 沿用 d:/ai share，其他平台落在用户主目录下，
// 避免在 macOS / Linux 上生成永远不存在的 d:\ 盘路径
const SEED_HOME = process.platform === 'win32' ? 'd:/ai share' : path.join(os.homedir(), 'ai share');
...
function seal(item, col) {
  ...
  for (const f of fields) {
    // 已是密文就不能再加密：否则 update / remove / restore / 同步等路径
    // 每落盘一次就给已有密文再套一层，字段会指数膨胀（见 crypto.isSealed 注释）
    if (out[f] != null && out[f] !== '' && !isSealed(out[f])) out[f] = encrypt(out[f]);
  }
  return out;
}
...
// fsync 目录：让 rename 本身也持久化（部分平台不支持，忽略即可）
...
// 把「读 → 改 → 写」整个序列放进写锁内。单进程里事件循环天然串行（CRUD 全同步），
// 不会丢更新；但若同时跑两个实例，两者的读改写会交错，后写完的一方会把对方的改动整体覆盖。
// 锁可重入，因此种子写入等嵌套调用不会把自己锁死。
function lockedWrite(col, fn) {
  const release = lock.acquire(fileFor(col));
  try {
    return fn();
  } finally {
    release();
...
// 原子落盘：写同目录临时文件 → fsync → rename（同分区 rename 是原子操作）。
// 原先直接 writeFileSync 覆盖目标文件，一旦写入过程中进程崩溃/磁盘写满，
// JSON 会被截断导致整个集合损坏。这里同时保留上一份完好数据用于崩溃回滚。
function writeRaw(col, data) {
  ensureDir();
  const target = fileFor(col);
  const tmp = target + '.' + process.pid + '.tmp'; // 带上 pid：多进程不会共用同一个临时文件
  const payload = Buffer.from(JSON.stringify(data.map(x => seal(x, col)), null, 2), 'utf8');
  ...
  fs.closeSync(fd);
  // 先把「当前这份完好数据」留作 .bak（即上一版本），再原子替换
...
function readRaw(col) {
  ...
  const f = fileFor(col);
  if (!fs.existsSync(f)) {
    lockedWrite(col, () => writeRaw(col, seed(col)));
    return seed(col);
  }
  try {
    return JSON.parse(fs.readFileSync(f, 'utf8'));
  } catch (e) {
    // 主文件损坏：先隔离证据（便于人工排查），再用最近一份完好的 .bak 回滚
    ...
    if (fs.existsSync(bak)) {
      try {
        ...
        return JSON.parse(fs.readFileSync(f, 'utf8'));
      } catch (e3) { /* 回滚失败继续走下面的兜底 */ }
    }
    // 无法恢复时退化为种子数据，避免单个文件损坏导致整个集合不可用
    lockedWrite(col, () => writeRaw(col, seed(col)));
    return seed(col);
  }
}
...
// 列表接口专用：不解密密钥字段（列表视图用不到明文，编辑表单走 GET 单条），
// 既省掉一次解密，也避免把明文密钥经 /api 或 MCP 桥交给调用方。
// 历史数据里某条 apiKey 膨胀到 2.3 MB，正是它把单个列表响应顶到 1.77 MB。
function listPublic(col) {
  const fields = SECRET_FIELDS[col];
  const rows = readRaw(col).filter(x => !x._deleted);
  if (!fields) return rows;
  return rows.map((x) => {
    const out = Object.assign({}, x);
    for (const f of fields) if (out[f] != null) out[f] = out[f] === '' ? '' : '__SET__';
    return out;
  });
}
function get(col, id) {
...
function remove(col, id, opts) {
  ...
  return lockedWrite(col, () => {
    ...
    // 墓碑保留完整原始记录（只打标记）：若裁剪成空壳，
    // ① 本地回收站恢复只能拿回空记录（字段全丢）；
    // ② LWW 同步按 id 整体替换，空壳墓碑传到对端会把对端的完整记录也替换掊（静默丢数据）。
    // 密钥落盘本就是密文，墓碑由 purgeTombstones 按保留期滚动清理，敏感残留风险可控。
...
// 删除恢复：撤销墓碑，刷新 updatedAt 以便通过同步传播（对端会重新合入该记录）。
function restore(col, id) {
  return lockedWrite(col, () => {
    const data = readRaw(col);
    const i = data.findIndex(x => x.id === id);
    if (i < 0) return null;
    data[i]._deleted = false;
    data[i].updatedAt = new Date().toISOString();
    writeRaw(col, data);
    return unseal(data[i], col);
  });
}
// 列出已删除（墓碑）记录，供「回收站」界面恢复。
function listDeleted(col) { return readRaw(col).filter(x => x && x._deleted).map(x => unseal(x, col)); }

// 永久清除某条墓碑（回收站「彻底删除」）。仅对 _deleted 记录生效；找不到返回 null。
function purgeTombstone(col, id) {
  return lockedWrite(col, () => {
    const data = readRaw(col);
    const i = data.findIndex(x => x && x._deleted && x.id === id);
    if (i < 0) return null;
    data.splice(i, 1);
    writeRaw(col, data);
    return { id };
...
/* ---------- 批量操作 ---------- */
// 批量删除：一次加锁完成 N 条墓碑标记，替代 N 次 HTTP 往返 + N 次加解锁。
function removeMany(col, ids) {
  const set = new Set((ids || []).filter((x) => typeof x === 'string' && x));
  if (!set.size) return { ok: true, removed: 0, missing: [] };
  return lockedWrite(col, () => {
    const data = readRaw(col);
    const now = new Date().toISOString();
    let removed = 0;
    for (const x of data) {
      if (x && !x._deleted && set.has(x.id)) { x._deleted = true; x.updatedAt = now; removed++; }
    }
    if (removed) writeRaw(col, data);
    const missing = Array.from(set).filter((id) => !data.some((x) => x && x.id === id));
    return { ok: true, removed, missing };
  });
}

// 按集合分组：items 为 [{ col, id }]，非法集合直接丢弃（返回空组，调用方无需再校验）。
function groupByCol(items) {
  ...
  for (const it of (items || [])) {
    ...
    groups.get(col).add(it.id);
  }
  return groups;
}

// 批量恢复：每组一次加锁，撤销墓碑并刷新 updatedAt 以便随同步传播。
function restoreMany(items) {
  const groups = groupByCol(items);
  let restored = 0;
  const missing = [];
  for (const [col, set] of groups) {
    lockedWrite(col, () => {
      const data = readRaw(col);
      const now = new Date().toISOString();
      for (const x of data) {
        if (x && x._deleted && set.has(x.id)) { x._deleted = false; x.updatedAt = now; restored++; }
      }
      writeRaw(col, data);
      for (const id of set) if (!data.some((x) => x && x.id === id)) missing.push(col + ':' + id);
    });
  }
  return { restored, missing };
}

// 批量彻底删除：只对墓碑生效，避免误删存活记录。
function purgeMany(items) {
  const groups = groupByCol(items);
  let purged = 0;
  const missing = [];
  for (const [col, set] of groups) {
    lockedWrite(col, () => {
      const data = readRaw(col);
      const kept = data.filter((x) => !(x && x._deleted && set.has(x.id)));
      if (kept.length !== data.length) { purged += data.length - kept.length; writeRaw(col, kept); }
      for (const id of set) if (kept.some((x) => x && x.id === id)) missing.push(col + ':' + id);
    });
  }
  return { purged, missing };
}

// 批量启用/停用：一次加锁切换多条记录的 enabled 字段（值未变化的记录不计入 changed）。
function setEnabledMany(col, ids, enabled) {
  const set = new Set((ids || []).filter((x) => typeof x === 'string' && x));
  if (!set.size) return { ok: true, changed: 0, missing: [] };
  return lockedWrite(col, () => {
    const data = readRaw(col);
    const now = new Date().toISOString();
    let changed = 0;
    for (const x of data) {
      if (x && !x._deleted && set.has(x.id) && !!x.enabled !== !!enabled) {
        x.enabled = !!enabled;
        x.updatedAt = now;
        changed++;
      }
    }
    if (changed) writeRaw(col, data);
    const missing = Array.from(set).filter((id) => !data.some((x) => x && !x._deleted && x.id === id));
    return { ok: true, changed, missing };
  });
}

// 清空回收站：遍历全部集合删除所有墓碑，返回按集合的计数（比前端逐条 purge 少 N-1 次加解锁）。
function purgeAllTombstones() {
  const byCollection = {};
  let total = 0;
  for (const col of COLLECTIONS) {
    lockedWrite(col, () => {
      const data = readRaw(col);
      const kept = data.filter((x) => !(x && x._deleted));
      const removed = data.length - kept.length;
      byCollection[col] = removed;
      total += removed;
      if (removed) writeRaw(col, kept);
    });
  }
...
// 逐层解开被重复加密的密钥字段：decrypt 对非密文原样返回，可据此安全定界。
function unwrapSecret(v) {
  let cur = v, layers = 0;
  while (isSealed(cur) && layers < 64) {
    const next = decrypt(cur);
    if (next === cur) return { layers, plain: cur, complete: false }; // 密钥不符，解不动了
    cur = next;
    layers++;
  }
  return { layers, plain: cur, complete: !isSealed(cur) };
}

// 体检 / 修复历史 bug 造成的「密钥被反复重复加密」（曾让某条 apiKey 膨胀到 3 MB）。
// 默认只扫描（apply 不为 true 即 dry run），只报告不落盘；无法自动修复的一律跳过，绝不破坏数据。
function repairSecrets(opts) {
  ...
  for (const col of COLLECTIONS) {
    ...
    lockedWrite(col, () => {
      const data = readRaw(col);
      let fixed = 0, unfixable = 0, savedBytes = 0;
      const samples = [];
      for (const item of data) {
        ...
        for (const f of fields) {
          ...
          const r = unwrapSecret(v);
          if (!r.complete) { unfixable++; continue; }
          if (r.layers < 2) continue; // 正常情况：单层，无需处理
          fixed++;
          savedBytes += Math.max(0, v.length - encrypt(r.plain).length);
          if (samples.length < 5) samples.push({ id: item.id, field: f, layers: r.layers, before: v.length });
          if (apply) item[f] = r.plain; // 交回 writeRaw 统一加密成单层
        }
      }
      if (fixed && apply) writeRaw(col, data);
      if (fixed || unfixable) report[col] = { fixed, unfixable, savedBytes, samples };
    });
  }
  return { apply, report };
}

function rewrite(col, items) { return lockedWrite(col, () => writeRaw(col, items)); }

// 墓碑 GC：清理超过保留期的删除墓碑，避免墓碑随删除次数无限增长。
// 保留期默认 30 天——足以让所有在线设备完成一次同步；过期后被旧设备重新推回的概率极低。
// days 必须为 >=1 的数字（不允许 0，避免一次性清空墓碑的误操作）。
function purgeTombstones(days) {
  const d = Number(days);
  if (!Number.isFinite(d) || d < 1) return { error: 'days 必须为 >=1 的数字' };
  ...
  for (const col of COLLECTIONS) {
    lockedWrite(col, () => {
      const data = readRaw(col);
      const kept = data.filter((x) => !(x && x._deleted && new Date(x.updatedAt).getTime() < cutoff));
      const removed = data.length - kept.length;
      if (removed > 0) {
        writeRaw(col, kept);
        total += removed;
  ...
  return { removed: total, byCollection, cutoff: new Date(cutoff).toISOString() };
}
...
function collectFromClients() {
  ...
  // 类型索引只需构建一次；新登记的客户端直接写入索引，不再重复读盘查询
  const byType = {};
  ...
  for (const d of dets) {
    ...
    if (!byType[d.type]) {
      const rec = create('clients', { name: d.name, type: d.type, enabled: true, configPath: d.configPath, description: '' });
      byType[d.type] = rec; clientsList.push(rec);
    }
...
function seed(col) {
  switch (col) {
    ...
    case 'mcpservers':
      return [
        { id: 'm_filesystem', name: 'filesystem', type: 'stdio', enabled: true, command: 'npx', args: ['-y', '@modelcontextprotocol/server-filesystem', SEED_HOME], env: {}, url: '', headers: {} },
    ...
    case 'skillrepos':
      return [
        ...
        { id: 's_local', name: '本地技能库', type: 'local', url: '', path: path.join(SEED_HOME, 'skills'), description: '本机维护的 skill 目录' }
      ];
    case 'clients':
      return [
    ...
    case 'repos':
      ...
      return [
        { id: 'r_repo_default', name: '生成的软件项目', type: 'local', url: '', path: path.join(SEED_HOME, 'repo'), resourceType: ['skillrepos', 'mcpservers', 'prompts'], enabled: true, description: '生成的软件项目目录（' + path.join(SEED_HOME, 'repo') + '），同步其中的 Skill/MCP/提示词' },
...
module.exports = { COLLECTIONS, DATA_DIR, purgeTombstones, list, listRaw, get, create, update, remove, restore, purgeTombstone, listDeleted, removeMany, restoreMany, purgeMany, setEnabledMany, purgeAllTombstones, listPublic, repairSecrets, rewrite, exportAll, restoreAll, exportProfileBundle, importProfileBundle, importScannedServers, importScannedSkills, importScannedPrompts, collectFromClients };
